...
import { NEW_TABLE_DATA_REF_QUERY } from "../../GraphQL/Queries";
...
const MainFlow = () => {
  ...
  const {
    loading: project_name_loading,
    error: project_name_error,
    data: project_name,
  } = useQuery(NEW_TABLE_DATA_REF_QUERY, {
    variables: { tableId: 513, tableColId: 1, tableRefId: 0, userId: userId },
  });

  const {
    loading: project_data_loading,
    error: project_data_error,
    data: project_data,
  } = useQuery(NEW_TABLE_DATA_REF_QUERY, {
  ...
  project_data?.getTableDataRefIdInfoUpdate?.map((item) => {
            ...
            <DialogContent>
              ...
              >
                <FormControl fullWidth size="small" sx={{ height: 32 }}>
                  ...
                  >
                    {project_name?.getTableDataRefIdInfoUpdate
