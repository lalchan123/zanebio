...
function ProcessLogDataShow() {
  ...
  const [processLogId, setProcessLogId] = useState("");
  ...
  const columns = [
    ...
    { field: "schedule", headerName: "Schedule", width: 90 },
      ...
      getActions: ({ id }) => {
        return [
          ...
          <GridActionsCellItem
            ...
            label="check"
            onClick={handleStatusDataCheck(id)}
            color="inherit"
          />,
        ];
      },
    ...
    const processLogFetchData = async () => {
      try {
        const response = await fetch(
          ...
          // "http://ec2-18-227-49-240.us-east-2.compute.amazonaws.com:8000/account/get-table-dynamic-field/542/"
  ...
  const handleStatusDataCheck = (id) => () => {
    setProcessLogId(id);
  };
  ...
  processLogData?.data?.map((item, i) => {
    if (item?.Process_Name === processNameCheck) {
      rows.push({
        id: item?.Process_Id,
  ...
  // console.log("check process id", processLogId);

  processStatusData?.process_log?.map((item, i) => {
    if (item?.process_id === processLogId) {
      rowsLog.push({
        id: item?.process_id,
        processid: item.process_id,
...
export default memo(ProcessLogDataShow);
