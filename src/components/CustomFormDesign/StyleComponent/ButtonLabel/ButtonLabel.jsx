...
const ButtonLabel = (props) => {
  ...
  const [APIUrl, setAPIUrl] = useState(item?.APIUrl || "");
  const [APIParameter, setAPIParameter] = useState(item?.APIParameter || "");
  const [APIOutputParameter, setAPIOutputParameter] = useState(item?.APIOutputParameter || "");
  // const [dataSourceId, setDataSourceId] = useState(item?.dataSourceId || "");
  const [dataSourceId, setDataSourceId] = useState(item?.dataSourceId || localStorage.getItem("dataSourceId"));
  ...
  let value = { name1: buttonLabel1, size: buttonSize, buttonColor1: buttonColor1, functionNameButton1: functionNameButton1, APIName: APIName, APIUrl:APIUrl, APIParameter: APIParameter, APIOutputParameter: APIOutputParameter, dataSourceId: dataSourceId };
        ...
        <div className="flex items-center ">
          <div className="flex items-center">
            <div className="mb-2 mr-2">
              ...
              <select
                id="function_name"
                class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500 text-xs"
                // class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 text-xs"
                value={APIName}
                onChange={(e) => setAPIName(e.target.value)}
              >
                <option selected className="text-xs">
                  {APIName ? APIName : "Choose API Name"}
                </option>
                {allApiData?.map((item, index) => { 
                  return (
                    <option
                      ...
                      value={item.api_name}
                      onClick={() => {
                        setAPIParameter(item.api_parameter)
                        setAPIUrl(item.api_url)
                      }
                      }
                    >
                      {item.api_name} == {item.api_url}
                    </option>
                  ) 
                })}
              </select>
          ...
          <div className="flex items-center">
            <div className="mb-2 mr-2">
              ...
              <textarea
                ...
                placeholder="Enter Api Input Parameter"
              />
            </div>
        </div>
      </div>  
          ...
          <div className="flex items-center">
            <div className="mb-2 mr-2">
              <label
                ...
                // class="block text-xs mb-0.5 font-mono text-gray-900 dark:text-white"
              >
                API Output Parameter
              </label>  
              <textarea
                className="w-full p-2 border rounded-lg"
                rows="5"
                value={APIOutputParameter}
                onChange={(e) => setAPIOutputParameter(e.target.value)}
                placeholder="Enter Api Output Parameter"
              />
            </div>
        </div>
      </div>  
      ...
      <div>
        ...
        <div class="inline-flex rounded-md shadow-sm">
          ...
          <a
            ...
            <a
              href="#"
              class="px-3 py-2 text-sm font-normal text-black bg-yellow-500 border border-gray-200 rounded-e-lg hover:bg-blue-800 hover:text-white focus:z-10 focus:ring-1 focus:ring-blue-700 focus:text-blue-700 bg-gray-700 border-gray-600 text-black hover:text-black hover:bg-gray-600 focus:ring-blue-500 focus:text-white no-underline"
