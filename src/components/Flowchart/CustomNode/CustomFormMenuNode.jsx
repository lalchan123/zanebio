...
function CustomFormMenuNode({
  ...
  const formItemList = ["input", "checkbox", "search", "button", "picture"];
  ...
  const [bodyItems, setBodyItems] = useState(data.body);

  const navigate = useNavigate();

  const saveMenuNodeData = (evt) => {
    const { nodes } = store.getState();
    setNodes(
      nodes.map((node) => {
        if (node.id === nodeId) {
  ...
  return (
    <>
      ...
      >
        ...
        <Box sx={{}} onClick={() => setItemShow(true)}>
          <Text
            style={{
              fontWeight: "250",
              fontSize: "7px",
        ...
        <DialogContent>
          <Box sx={{ minWidth: 300, minHeight: 80 }}>
            <input
              ...
              name="price"
              id="price"
              className="block w-[300px] rounded-md border-0 py-1.5 pl-7 pr-20 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6"
              placeholder=""
      ...
      >
        <DialogContent>
          <FormControl
            sx={{ m: 1, minWidth: 500, minHeight: 200 }}
            size="small"
            color="success"
          >
            <InputLabel
            ...
            >
              {formItemList?.map((name) => (
                ...
                >
                  <Checkbox
                    checked={formItem.indexOf(name) > -1}
                    size="small"
                    color="success"
                  />
                  <ListItemText primary={name} size="small" />
                </MenuItem>
              ))}
            </Select>
          </FormControl>
...
export default memo(CustomFormMenuNode);
