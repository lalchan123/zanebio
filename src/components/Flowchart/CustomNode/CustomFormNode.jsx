...
function CustomFormNode({
  ...
  const onBodyButton = () => {
    setCustomFormShow(true);
    setCustomFormDesgin(data?.form?.formDesign);
    navigate("/customform");
  };

  const saveFormNodeData = (evt) => {
    const { nodes } = store.getState();
    setNodes(
      nodes.map((node) => {
        if (node.id === nodeId) {
          node.data = {
            ...node.data,
            form: { formDesign: customFormDesgin },
          };
        }
        return node;
      })
      ...
      >
        ...
        <Tooltip title="Save" placement="top">
          <IconButton
            ...
            aria-label="delete"
            size="small"
            onClick={saveFormNodeData}
            onChange={saveFormNodeData}
            value={"demo"}
            nodeId={id}
          >
            <Box
