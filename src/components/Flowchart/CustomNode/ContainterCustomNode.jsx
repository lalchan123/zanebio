...
function ContainterCustomNode({
  ...
  const saveContainerNodeData = (evt) => {
    const { nodes } = store.getState();
    setNodes(
      nodes.map((node) => {
        if (node.id === nodeId) {
          node.data = {
            ...node.data,
            container: { processType: processType },
          };
        }
        return node;
      })
      ...
      >
        ...
        <Tooltip title="Save" placement="top">
          <IconButton
            ...
            aria-label="delete"
            size="small"
            onClick={saveContainerNodeData}
            onChange={saveContainerNodeData}
            value={"demo"}
            nodeId={id}
          >
            <Box
...
export default memo(ContainterCustomNode);
