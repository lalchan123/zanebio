...
import { FILE_NAME_LIST_QUERY } from "../../../GraphQL/Queries";
...
function CustomNodeFIleUpload({
  ...
  const filedata = file_name?.getYahooHistFileName?.jsonData;
  const fileNameJson = eval(filedata);

  const [filterData, setFilterData] = useState(
    eval(file_name?.getYahooHistFileName?.jsonData)
  );

  const fileNameFilter = () => {
    const searchText = searchItem.toLowerCase();
    const filteredList = filterData.filter((name) =>
      name.toLowerCase().includes(searchText)
    );
    setFilterData(filteredList);
  };

  const saveFileUploadNodeData = (evt) => {
    const { nodes } = store.getState();
    setNodes(
      nodes.map((node) => {
        if (node.id === nodeId) {
      ...
      >
        ...
        <Tooltip title="Save" placement="top">
          <IconButton
            ...
            aria-label="delete"
            size="small"
            onClick={saveFileUploadNodeData}
            onChange={saveFileUploadNodeData}
            value={"demo"}
            nodeId={id}
          >
            <Box
            ...
            <Box sx={{ minWidth: 500, minHeight: 200 }}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  ...
                  sx={{ fontSize: "14px" }}
                >
                  {/* <MenuItem size="small" value={"rectangular"}></MenuItem> */}
                  <Input
                    sx={{ maxWidth: 240, margin: 2 }}
                    onClose={handleCloseFile}
                    value={searchItem}
...
export default memo(CustomNodeFIleUpload);
